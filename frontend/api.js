...
$(document).ready(function () {
    ...
    species.autocomplete({
        source: (request, response) => {
            $.get("api/search/species", { query: APP.species.name })
                .done(function (data) {
                    data.forEach((e) => {
    ...
    protein.autocomplete({
        source: (request, response) => {
            if (APP.species.ncbi_id == null) {
                alert("Select the species first!")
                return;
            }

            $.get("api/search/protein", { query: APP.protein.name, species_id: APP.species.ncbi_id })
                .done(function (data) {
                    data.forEach((e) => {
    ...
    protein_btn.click(() => {
        ...
        $.get("api/subgraph/protein", { protein_id: APP.protein.id, threshold: threshold })
    ...
    pathway.autocomplete({
        source: (request, response) => {
            ...
            $.get("api/search/pathway", { query: APP.pathway.name, species_id: APP.species.ncbi_id })
                .done(function (data) {
                    data.forEach((e) => {
    ...
    pathway_btn.click(() => {
        $.get("api/subgraph/pathway", { pathway_id: APP.pathway.id })
            .done(function (subgraph) {
    ...
    protein_list_btn.click(() => {
        ...
        $.get("api/search/protein_list", { query: APP.protein_list.value.split('\n').join(';'), species_id: APP.species.ncbi_id })
                .done(function (data) {
                    APP.protein_list.ids = data.map(x => x.protein_id);

                    var threshold = parseFloat(APP.threshold.value);
                    $.get("api/subgraph/protein_list", { protein_ids: APP.protein_list.ids.join(';'), threshold: threshold })
                        .done(function (subgraph) {
