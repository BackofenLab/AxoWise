...
export default function saveAsSVG(renderer, params, mode) {
  ...
  // Creating a camera
  var camera = renderer.addCamera();

  // Creating a svg renderer
  var SVGrenderer = renderer.addRenderer({
    camera: camera,
    container: container,
    type: "svg",
    settings: {
  ...
  if (mode == "white") {
    SVGrenderer.settings({ defaultLabelColor: "#000" });
  }
  ...
  renderer.killRenderer(SVGrenderer);
  renderer.killCamera(camera);
