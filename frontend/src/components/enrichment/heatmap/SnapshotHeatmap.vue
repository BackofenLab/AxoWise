...
export default {
    ...
    methods: {
        findSnapshot(){
            return this.$store.state.snapshot_heatmaps.find(dictionary => dictionary.id === this.index)
        },
        ...
        getSnapshot(data) {
            ...
            this.$store.commit('assign_snapshotHeatmap', { id: this.index, snapshot: image})
