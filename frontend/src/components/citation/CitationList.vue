        ...
        <div class="pathwaybar">
            <div id="citation-list">
                ...
                <div class="list-section">
                    ...
                    <div class="sorting">
                        <a class="pubid_filter" v-on:click="sort_alph = (sort_alph === 'asc') ? 'dsc' : 'asc'; sort_pr = ''; sort_cb = ''; sort_y = '' ">Pubmed ID</a>
                        <a class="cited_filter" v-on:click="sort_cb = (sort_cb === 'asc') ? 'dsc' : 'asc'; sort_pr = ''; sort_alph = ''; sort_y = '' " >citation</a>
                        <a class="year_filter" v-on:click="sort_y = (sort_y === 'asc') ? 'dsc' : 'asc'; sort_pr = ''; sort_cb = ''; sort_alph = '' " >year</a>
                    ...
                    <div class="results" tabindex="0" @keydown="handleKeyDown" ref="resultsContainer">
                        <table >
                            <tbody>
                                <tr v-for="(entry, index) in filt_abstracts" :key="index" class="option" :class="{ selected: selectedIndex === index }" v-on:click="select_abstract(entry, index)">
                                    <td>
...
export default {
    ...
    computed: {
        ...
        filt_abstracts() {
            ...
            if(com.sort_cb == "asc"){
                filtered.sort((t1, t2) => t2.attributes["Citation"] - t1.attributes["Citation"])
            }else if (com.sort_cb == "dsc"){
                filtered.sort((t1, t2) => t1.attributes["Citation"] - t2.attributes["Citation"])
            }

            if(com.sort_y == "asc"){
                filtered.sort((t1, t2) => t2.attributes["Year"] - t1.attributes["Year"])
            }else if (com.sort_y == "dsc"){
                filtered.sort((t1, t2) => t1.attributes["Year"] - t2.attributes["Year"])
            }

            if(com.sort_pr == "asc"){
                filtered.sort((t1, t2) => t2.attributes["PageRank"] - t1.attributes["PageRank"])
            }else if (com.sort_pr == "dsc"){
                filtered.sort((t1, t2) => t1.attributes["PageRank"] - t2.attributes["PageRank"])
            }
