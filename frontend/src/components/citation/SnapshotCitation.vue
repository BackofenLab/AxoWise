...
export default {
    ...
    methods: {
        findSnapshot(){
            return this.$store.state.snapshot_citations.find(dictionary => dictionary.id === this.index)
        },
        initializeSnapshot(data) {
            const imageContainer = document.getElementById('citation-visual-' + this.index)
        ...
        getSnapshot(data) {
            ...
            var image = saveAsSVG(sigma_instance, {download:false, width: 200, height: 200})
            this.$store.commit('assign_snapshotCitation', { id: this.index, snapshot: image})
