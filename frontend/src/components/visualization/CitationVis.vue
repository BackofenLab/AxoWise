...
getCircleStyle(circle){
  ...
  mounted() {
    ...
    this.emitter.on("searchNode", (state) => {
      if(state.mode=="citation") this.$emit('active_node_changed', sigma_instance.graph.getNodeFromIndex(state.node.id))
    });
    
    this.emitter.on("searchSubset", (state) => {
      if(state.mode=="citation") this.$emit('active_subset_changed', state.subset)
    });
    ...
    this.emitter.on("hideSubset", (state) => {
      if(state.mode=="citation") this.$emit('active_layer_changed', state.subset)
    });
    ...
    this.emitter.on("deactivateModules", (state) => {
      if(state.mode=="citation") com.moduleSelectionActive = !com.moduleSelectionActive
    });
