...
export default {
  ...
  watch: {
    ...
    active_node(node) {
      ...
      com.focusNode(
        sigma_instance.cameras[0],
        sigma_instance.graph.getNodeFromIndex(node.id)
      );
      ...
      if (com.threeview) {
        if (node.active) {
          const nodesById = Object.fromEntries(
            three_instance
              .graphData()
              .nodes.map((node3d) => [node3d.id, node3d])
          );
      ...
      if (com.active_termlayers != null) {
        ...
        for (const edge of graph.edges()) {
          ...
          if (proteinList.has(edge.source)) {
            ...
            edge.color = com
              .get_normalize(source_value, -1, 1)
              .replace(")", "," + com.base_opacity + ")")
              .replace("rgb", "rgba");
          } else {
          ...
          if (proteinList.has(edge.target)) {
            ...
            edge.color = com
              .get_normalize(target_value, -1, 1)
              .replace(")", "," + com.base_opacity + ")")
              .replace("rgb", "rgba");
          } else {
    ...
    active_termlayers: {
      handler(newList) {
        ...
        const filteredArray = new Set(
          visibleTermlayers.filter((value) => !hiddenTermLayer.has(value))
        );
        ...
        sigma_instance.graph.edges().forEach((e) => {
          ...
          if (proteinList.has(e.source) && proteinList.has(e.target)) {
            e.color = source.color
              .replace(")", ", " + this.highlight_opacity + ")")
              .replace("rgb", "rgba");
            highlighted_edges.add(e);
    ...
    active_combine(val) {
      ...
      if (val.name == "devalue")
        this.$emit("active_decoloumn_changed", val.value);
      if (val.name == "layers")
        this.$emit("active_termlayers_changed", val.value);
    },
  },
  methods: {
    ...
    async three_view() {
      ...
      var newNodeList = Array.from(
        com.gephi_data.nodes.filter((node) => subgraphSet.has(node.id))
      );

      const newEdges = Array.from(
        com.gephi_data.edges.filter(
          (edge) => subgraphSet.has(edge.source) || subgraphSet.has(edge.target)
        )
      );
    ...
    update_boundary: function (data) {
      ...
      sigma_instance.graph.edges().forEach(function (e) {
        ...
        source.color = com.get_normalize(
          source.attributes[com.active_decoloumn],
          minBound,
          maxBound
        );
        target.color = com.get_normalize(
          target.attributes[com.active_decoloumn],
          minBound,
          maxBound
        );
        e.color = com
    ...
    visualize_pathway(startID, endID) {
      ...
      const paths = new Set(
        sigma_instance.graph.astar(startNode.id, endNode.id)
      );
    ...
    highlight_de(value) {
      ...
      var unconnected_nodes = new Set(
        com.unconnected_nodes.map((n) => n.attributes["Ensembl ID"])
      );
      sigma_instance.graph.nodes().forEach((n) => {
        if (
          checkSet.has(n.attributes["Ensembl ID"]) &&
          !(
            unconnected_nodes.has(n.attributes["Ensembl ID"]) && com.graph_state
          )
        ) {
          n.hidden = false;
        } else n.hidden = true;
      ...
      sigma_instance.graph.nodes().forEach((n) => {
        if (moduleSet[n.attributes["Modularity Class"]])
          ...
          moduleSet[n.attributes["Modularity Class"]] = [
            { x: n["renderer1:x"], y: n["renderer1:y"] },
          ];
      });
