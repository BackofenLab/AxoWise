<template>
  ...
  export default {
    ...
    computed: {
        ...
        return ( this.windowcheck && route !== 'home' && route !== 'input' && route !== 'file' && route !== 'import'); // Example: Hide component on 'View3'
        }
    },
    mounted(){
        this.dragElement(document.getElementById("genchatbot"));
    },
    methods: {
        dragElement(elmnt) {
            var pos1 = 0,
            ...
            function elementDrag(e) {
                ...
                if (newTop + elementHeight > parentHeight) newTop = parentHeight - elementHeight;
                if (newLeft + elementWidth > parentWidth) newLeft = parentWidth - elementWidth;
      ...
      sendMessage() {
        if (this.userInput.trim() !== '') {
          ...
          // Add bot response
          setTimeout(() => {
            this.messages.push({ sender: 'Bot', text: 'Let me get back to you on that!' });
          }, 1000);
        }
      },
      closeWindow() {
