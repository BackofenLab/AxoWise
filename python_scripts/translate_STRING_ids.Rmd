...
```{r ensem_to_entrez}
string_SYMBOL_ENSEMBL <- fread("../data/string_SYMBOL_ENSEMBL.tsv.gz", sep = "\t", header = TRUE)
string_SYMBOL_ENSEMBL %>% head(1)

# convert ENSEMBL IDs in exp_DE to ENTREZID
ensembl_entrez <- mapIds(org.Mm.eg.db,
       keys=string_SYMBOL_ENSEMBL$ENSEMBL, #Column containing Ensembl gene ids
       column="ENTREZID",
       keytype="ENSEMBL",
       multiVals="first")

string_SYMBOL_ENSEMBL$ENTREZID <-  ensembl_entrez[string_SYMBOL_ENSEMBL$ENSEMBL]
# replace NULL values in the list column with "NA"
string_SYMBOL_ENSEMBL$ENTREZID <- lapply(string_SYMBOL_ENSEMBL$ENTREZID, function(x) ifelse(is.null(x), "NA", x))

fwrite(string_SYMBOL_ENSEMBL, "../data/string_SYMBOL_ENSEMBL.tsv.gz", sep = "\t", row.names = FALSE, quote = FALSE, na = 'NA', compress = "gzip")
```
