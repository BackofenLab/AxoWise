...
class TestSQLQueries(unittest.TestCase):
    ...
    def test_get_proteins(self):
        ...
        proteins = list(SQL.get_proteins(postgres_connection, species_id = 10090))
        self.assertGreater(len(proteins), 0)

        entered_if_1 = False
        entered_if_2 = False
        for protein in proteins:
            if protein["preferred_name"] == "Ccl5":
                entered_if_1 = True
                self.assertEqual(protein["id"], 2100220)
                self.assertEqual(protein["external_id"], "10090.ENSMUSP00000039600")
                self.assertEqual(protein["preferred_name"], "Ccl5")
                self.assertEqual(
                    protein["annotation"],
    ...
    def test_get_associations(self):
        ...
        associations = list(SQL.get_associations(postgres_connection, species_id = 10090))
