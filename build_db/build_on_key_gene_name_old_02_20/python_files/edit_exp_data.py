...
def rel_same_entity(data: str, target_dir: str):
    # read the data into panda frame
    df = pd.read_csv(data)
    # filter for proteins, where protein is transcription factor
    df = df[df['TF'] != "no"]
    ...
    df = df[['SYMBOL']]
    df.to_csv(target_dir + '/rel_same_entity.csv', index=False)
...
def rel_de(data: str, target_dir: str):
    ...
    df3.to_csv(target_dir + '/rel_protein_wt336h_wt0h', index=False)
    # see line 68
...
def rel_da(data: str, target_dir: str):
    ...
    df3 = df3.assign(Study='1', Celltype='Microglia')
    df3.to_csv(target_dir + '/rel_or_wt336h_wt0h', index=False)
    # see line 113
...
if __name__ == "__main__":
    ...
    source_dir = parent_dir + '/original_data/exp_data/'
    # define the path to the target diretory
    target_dir = parent_dir + '/edited_data/'
    exp_DE = source_dir + 'exp_DE.csv'
    exp_DA = source_dir + 'exp_DA.csv'
    # run the functions
    protein(exp_DE, target_dir)
    transcription_factor(exp_DE, target_dir)
    rel_de(exp_DE, target_dir)
    rel_nearest_distance(exp_DA, target_dir)
    open_region(exp_DA, target_dir)
    rel_da(exp_DA, target_dir)
    print(f"Edited differential expression files written to: {target_dir}")
