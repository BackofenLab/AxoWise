"""
This creates the data for the toy example.
"""
...
if __name__ == '__main__':
    ...
    for i in range(10):
        df = pd.read_csv(("../edited_data/" + relationships[i]),nrows=number_rows)
        df.to_csv(("../final_data/" + relationships[i]), index= None)
    df.rename(columns={"nearest_index": "name"}, inplace=True)
    OR = OR.append(df[~df['name'].isin(OR['name'])], ignore_index=True)
    
    # peak_target_cor.csv
    df = pd.read_csv(("../edited_data/" + relationships[21]), nrows=number_rows, sep='\t')
    df.to_csv(("../final_data/" + relationships[21]), index= None)
    df.rename(columns={'nearest_index': 'name'}, inplace=True)
    OR = OR.append(df[~df['name'].isin(OR['name'])], ignore_index=True)
    
    # TF_motif_peak.csv
    df = pd.read_csv(("../edited_data/" + relationships[22]), nrows=number_rows, sep='\t')
    ...
    for i in range(10,20):
        df = pd.read_csv(("../edited_data/" + relationships[i]),nrows=number_rows)
        df.to_csv(("../final_data/" + relationships[i]), index= None)
    Proteins = Proteins.append(df[~df['SYMBOL'].isin(Proteins['SYMBOL'])], ignore_index=True)

    # TF_target_cor.csv
    df = pd.read_csv(("../edited_data/" + relationships[20]), nrows=number_rows, sep='\t')
    df.to_csv(("../final_data/" + relationships[20]), index= None)

    Proteins = Proteins.append(df[~df['SYMBOL'].isin(Proteins['SYMBOL'])], ignore_index=True)
    # rel_string_functional_terms_to_proteins.csv
    df = pd.read_csv(("../edited_data/" + relationships[23]), nrows=number_rows)
    ...
    # rel_string_proteins_to_proteins.csv
    df = pd.read_csv(("../edited_data/" + relationships[25]), nrows=number_rows)
    ...
    df = pd.read_csv(("../edited_data/" + relationships[23]), nrows=number_rows)
    df.to_csv(("../final_data/" + relationships[23]), index= None)
    FunctionalTerms = FunctionalTerms.append(df[~df['term'].isin(FunctionalTerms['term'])], ignore_index=True)

    # "KappaEdges.csv"
    df = pd.read_csv(("../edited_data/" + relationships[24]), nrows=number_rows)
    ...
    TranscriptionFactors = pd.DataFrame({'TF':[]})
     ...
     # TF_target_cor.csv
    df = pd.read_csv(("../edited_data/" + relationships[20]), nrows=number_rows, sep='\t')
    TranscriptionFactors = TranscriptionFactors.append(df[~df['TF'].isin(TranscriptionFactors['TF'])], ignore_index=True)
